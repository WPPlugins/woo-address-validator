...
class WCAV_WC {
	 ...
	 */
	public function setup( $plugin_path, $plugin_url, WCAV_Validator $validator ) {
	...
	/**
	 * Validate addresses, when they are eddited on the myaccount page.
	 *
	...
	public function validate( $posted ) {
		...
		if ( isset( $posted['billing_address_1'] ) ) {
			...
			if ( ! $this->is_valid_billing && $is_first_round ) {
				$this->wc_notice();
				if ( 'SUSPECT' === $this->validator->get_status() ) {
					$set_cookie = TRUE;
