...
class WCAV_Admin {
	...
	public function setup( $plugin_path, $plugin_url, $validator ) {
		...
		if (
			...
			&& ! empty( $_POST[ $this->action_name ] ) // Input var okay.
			&& wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ $this->action_name ] ) ), $this->action ) // Input var okay.
		) {
			$data = (array) wp_unslash( $_POST['wcav'] ); // Input var okay.
			if ( $this->update( $data ) ) {
	...
	public function render() {
		...
		<div class="wrap">
			...
			<?php if ( ! empty( $_GET['key-invalid'] ) && 1 === (int) $_GET['key-invalid'] ) : // Input var okay. ?>
				<p>
					<?php esc_html_e( 'It looks like your API key is invalid or depleted. Please check your key and refresh it.', 'woo-address-validator' ); ?>
					<?php echo wp_kses_post( sprintf( __( 'You can get a new key <a href="%s">here</a>.', 'woo-address-validator' ), 'https://www.address-validator.net/' ) ); ?></p>
				</p>
			<?php endif; ?>
			...
			<p>
				<?php
				esc_html_e(
					'With our USPS address verification and international address validation service you can easily verify if any national or international postal address is valid and really exists. Stop wrong shipping or billing addresses from getting in your Woocommerce customer database.', 'woo-address-validator'
				);
				?><br>
				<?php
				echo wp_kses_post(
					sprintf(
						__(
							'You can use our Address Online Validation API to check up to 100 addresses for free - just <a href="%1$s" target="_blank">sign up here for your free API key</a> or inform yourself <a href="%2$s" target="_blank">about our low prices</a>.',
							'woo-address-validator'
						),
						'https://www.address-validator.net/address-validation-online-api-get-free-trial.html',
						'https://www.address-validator.net/address-verification-pricing.html'
					)
				);
				?>
			</p>
	...
	public function render_invalid_key_notice() {
		...
		<div class="notice notice-error is-dismissible">
			<p>
				...
				if ( empty( $_GET['page'] ) || 'wcav' !== sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) { // Input var okay.
					echo '<br>' . wp_kses_post(
							sprintf(
								__(
									'Update it <a href="%s">on the settings page</a>.',
									'woo-address-validator'
								),
								$url
							)
